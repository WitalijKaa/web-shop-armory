export default function CartItemSimple({ item, ix }) {
  return (
    <div className="w-[600px] rounded-2xl border border-[#19140035] bg-white p-6 shadow-sm my-8 ml-8">
      ...
      <h4 className="text-4l font-bold text-center my-4">{item.product_item.product.price.toFixed(2)} $</h4>
      <h4 className="text-4l text-center my-4">reserved {item.amount}</h4>
    </div>
  )
}
