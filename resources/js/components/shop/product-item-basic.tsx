...
export default function ProductItemBasic({ item, ix, allowRemove = false }) {
  let priceInCart = item.inCartItem ? item.inCartItem.amount * item.product.price : item.product.price;
  let amountAvailable = item.inCartItem ? item.amount - item.inCartItem.amount : item.amount;

  return (
    <div className="w-[400px] rounded-2xl border border-[#19140035] bg-white p-6 shadow-sm">
      ...
      {item.inCartItem && item.inCartItem?.amount > 0 && (
        <h4 className="text-4l text-center my-4">
          in cart {item.inCartItem.amount}
          {item.inCartItem?.amount > 1 && (
